...
object GeoTrans {
    ...
    fun tm2geo(srctype: Int, in_pt: GeoTransPoint, out_pt: GeoTransPoint) {
        ...
        if (m_Ind[srctype] != 0.0) {
            ...
            val g = 0.5 * (f - 1.0 / f)
            val temp = m_arLatCenter[srctype] + tmpPt.y / (m_arMajor[srctype] * m_arScaleFactor[srctype])
            val h = Math.cos(temp)
        ...
        var i = 0
        while (true) {
            val delta_Phi = (con + e1fn(m_Es[srctype]) * sin(2.0 * phi) - e2fn(
                m_Es[srctype]
            ) * sin(4.0 * phi) + e3fn(m_Es[srctype]) * sin(6.0 * phi)) / e0fn(
                m_Es[srctype]
            ) - phi
            phi += delta_Phi
